 ...
 * @author YONY ACEVEDO
 * @description Controlador que se encarga de procesar las solicitudes relacionadas a las Ventas 
 */
...
class InvoicesController extends Controller{
    ...
    public function index(){
        ...
        'js/Invoices/Invoices.js',
        'js/custom.js'
        );

        $this->view->app->css = array(
        'css/Invoices/Invoices.css',
        'css/default.css' 
        );
        
        $this->view->title = 'Listado de Ventas';
    ...
    public function add(){
        ...
        $this->view->title = 'Nueva Venta';
        
        $items = json_decode(file_get_contents($this->userPath.'/items_in_invoices.json'), JSON_OBJECT_AS_ARRAY);
                    
        $quantity = array_count_values(array_map(function($foo){return $foo['catalog_code'];}, $items['invoices_items']));

        foreach ($items['invoices_items'] as $key => $value) {
        ...
        $this->view->invoices_items['invoices_items'] = $data;
    ...
    public function invoices(){
        ...
        if(!count($invoices['Invoices']['customer'])){
