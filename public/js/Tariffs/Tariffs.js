...
localStorage.setItem("v_menu_active_id", "v_menu_tariffs");
localStorage.setItem("h_menu_active_id", "h_menu_tariffs");
localStorage.setItem("op_menu-open", "op_principal");          

/*INICIALIZACION DE COMPONENTE DATATABLE PARA LA TABLA CATALOG*/    

    var tabla = $('#tariffstable').DataTable( {
        "iDisplayLength": 5,
        "sDom": 'rtip',
        "filter": true,
        "info": true,
    ...
    $('#search').on('click', function () {
        ...
        //alert($('#select_status :selected').val());
        ...
        if(input_empty>0){
            ...
            tabla.column().columns(4).search($('#select_status :selected').val());
            tabla.draw();
            tabla.ajax.reload();
